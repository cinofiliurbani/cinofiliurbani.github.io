...
class CP_Before_After extends cp_Framework {
	 ...
	 * @var slug
	 */
	public static $slug = 'before_after';
	...
	public function __construct() {
		self::$settings = array(
			'title'       => __( 'Before/After', 'convertpro' ),
			'description' => __( 'A banner or an opt-in form that can be embedded before or after your blog post on a page.', 'convertpro' ),
		);
	...
	/**
	 * Function Name: hide_display_inline_option.
	 * Function Description: This will modify configuration options.
	 *
	 * @param Array $options Options array.
	 */
	public function hide_display_inline_option( $options ) {
		...
		foreach ( $options as $key => $option ) {
			...
			if ( 'inline_position' === $option['name'] ) {
				unset( $option['dependency'] );
			}
	...
	public function get_options() {
		...
		$options = parent::cp_remove_panel( array( 'Cookies', 'Embed' ), $options );
		...
		$options = parent::cp_remove_field_options(
			...
			array( 'title', 'font_family', 'font_size', 'text_color', 'text_hover_color', 'back_color', 'back_color_hover', 'letter_spacing', 'btn_text_align', 'line_height', 'field_padding', 'respective_to', 'is_outside_hide', 'label_position' ),
			$options
		);
	...
	public function get_design_options() {
		...
		$panel_design_options = array(
			...
			array(
				...
				'name'         => 'panel_position',
				'opts'         => array(
					'title'       => __( 'Design Position', 'convertpro' ),
					'value'       => 'middle',
					'description' => __( 'This position will be set for entire call-to-action. This will affect on frontend only.', 'convertpro' ),
					'options'     => array(
						'left'   => __( 'Left', 'convertpro' ),
						'middle' => __( 'Center', 'convertpro' ),
		...
		/*** Array contains Advance design field options */
